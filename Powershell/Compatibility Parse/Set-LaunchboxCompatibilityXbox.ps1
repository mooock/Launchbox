Function Set-LaunchboxCompatibilityXbox {
param (
$Name,
$Compatibility = 'Perfect',
$Path = 'E:\Launchbox\Launchbox\Data\Platforms\Microsoft Xbox.xml',
$CustomfieldName = 'Microsoft Xbox Compatibility',
[Switch]$WriteData
)

    Function Get-CXBXCompatibilitylist {
param (
    $pages = 43,
    $Threads = 3
)
...
$uris = 1..$pages | ForEach-Object {
    "https://cxbx-reloaded.co.uk/compatibility?page=$_"
}

$i = 0
$uris | Invoke-Parallel -MaxQueue $Threads -ScriptBlock {
    $uri = $_
    $pagecount++
    $webrequest = Invoke-WebRequest -Uri $uri
    [ARRAY]$data =  ($webrequest.ParsedHtml.IHTMLDocument3_documentElement.getElementsByClassName('d-flex w-100 justify-content-between')) | select innertext
    
    $i=1
    $itemcount = $data.innertext.count / 3
    0..($itemcount -1)  | foreach {
 
            [PSCUSTOMOBJECT]@{
                Title         = ($data.innertext[($_*3)] -split "`r`n" | select -First 1).TrimEnd()
                Version       = ($data.innertext[($_*3+1)] -split ":" | select -First 1).TrimEnd()
                Compatibility = ($data.innertext[($_*3+2)]).TrimEnd()
            }
        } 
    }
}
    $Complist = Get-CXBXCompatibilitylist -pages 42
    $ComplistUnique = $Complist | where Compatibility -NotLike 'Untested' | Sort-Object Title -Unique | select -Skip 1
    ...
    ForEach ($Customfield in $data.LaunchBox.CustomField){
        ...
        $WebCompatibility = 'Missing Data'
        $ComplistUnique | ForEach-Object {
            $FuzzySearch = $_.'Title' -replace ': ',' '
